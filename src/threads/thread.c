...
/*
 * List of all processes.  Processes are added to this list when they are
 * first scheduled and removed when they exit.
 */
 ...
 * running into a thread.  This can't work in general and it is possible in
 * this case only because loader.S was careful to put the bottom of the stack
 * at a page boundary.
 *
...
/*
 * Starts preemptive thread scheduling by enabling interrupts. Also creates
 * the idle thread.
 */
...
/*
 * Called by the timer interrupt handler at each timer tick. Thus, this
 * function runs in an external interrupt context.
 */
...
/*
 * Transitions a blocked thread T to the ready-to-run state. This is an error
 * if T is not blocked.  (Use thread_yield() to make the running thread
 * ready.)
 *
...
thread_current(void)
{
  ...
  /*
   * Make sure T is really a thread. If either of these assertions
   * fire, then your thread may have overflowed its stack.  Each thread
   * has less than 4 kB of stack, so a few big automatic arrays or
   * moderate recursion can cause stack overflow.
   */
  ASSERT(is_thread(t));
...
/*
 * Deschedules the current thread and destroys it.  Never returns to the
 * caller.
 */
void thread_exit(void)
{
  ...
  /*
   * Remove thread from all threads list, set our status to dying, and
   * schedule another process.  That process will destroy us when it
...
/*
 * Invoke function 'func' on all threads, passing along 'aux'. This function
 ...
 *
 * The idle thread is initially put on the ready list by thread_start().  It
 * will be scheduled once initially, at which point it initializes
 * idle_thread, "up"s the semaphore passed to it to enable thread_start() to
 * continue, and immediately blocks.  After that, the idle thread never
...
running_thread(void)
{
  ...
  /*
   * Copy the CPU's stack pointer into `esp', and then round that down
   * to the start of a page.  Because `struct thread' is always at the
   * beginning of a page and the stack pointer is somewhere in the
   * middle, this locates the curent thread.
   */
  asm("mov %%esp, %0" : "=g"(esp));
...
/*
 * Allocates a SIZE-byte frame at the top of thread T's stack and returns a
...
void thread_schedule_tail(struct thread *prev)
{
   ...
   * thread.  This must happen late so that thread_exit() doesn't pull
   * out the rug under itself.  (We don't free initial_thread because
   * its memory was not obtained via palloc().)
   */
...
/*
 * Offset of `stack' member within `struct thread'. Used by switch.S, which
